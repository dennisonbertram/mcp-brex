...
interface GetBudgetParams { budget_id: string; summary_only?: boolean; fields?: string[]; }
...
function validateParams(input: unknown): GetBudgetParams {
  ...
  const out: GetBudgetParams = { budget_id: String(raw.budget_id) };
...
export function registerGetBudgetById(_server: Server): void {
  registerToolHandler("get_budget", async (request: ToolCallRequest) => {
    try {
      ...
      const tooBig = estimateTokens(JSON.stringify(budget)) > 24000;
      const summarized = params.summary_only || tooBig;
      const output = summarized ? project(budget, fields) : budget;
      return { content: [{ type: "text", text: JSON.stringify({ budget: output, meta: { summary_applied: summarized } }, null, 2) }] };
    } catch (error) {
